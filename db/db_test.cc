  ...
  Status NewRandomAccessFile(const std::string &f, RandomAccessFile **r) {
    class CountingFile : public RandomAccessFile {
     ...
     public:
      ...
      virtual Status Mmap(uint64_t offset, size_t n, Slice *result,
                          char *&scratch) const {
        return Status::OK();
      }
      virtual Status UnMmap(size_t n, char *scratch) const {
        return Status::OK();
      }
    };
    ...
    for (int run = 0; run < 3; run++) {
      ...
      for (int compact_start = 0; compact_start < N; compact_start += 10) {
        for (int i = 0; i < N; i += 10) {
          ...
          ASSERT_TRUE(
              Between(Size("", Key(i) + ".suffix"), S1 * (i + 1),
                      S2 * (i + 1)));
...
TEST(DBTest, ManifestWriteError) {
  ...
  for (int iter = 0; iter < 2; iter++) {
    port::AtomicPointer *error_type =
        (iter == 0) ?
            &env_->manifest_sync_error_ : &env_->manifest_write_error_;
    ...
    if (rnd.OneIn(2)) {
      ...
      if (s.IsNotFound()) {
        ...
        ASSERT_LE(
            ...
            reinterpret_cast<uintptr_t>(t->state->counter[w].Acquire_Load()));
      }
    }
    counter++;
  }
