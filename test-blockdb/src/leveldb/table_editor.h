...
class TableEditor {
 public:
  ...
  TableEditor(const Options &options, const std::string dbname,
              uint64_t file_number, uint64_t file_offset, uint64_t level,
              std::string *old_filter, char *buffer);

  // REQUIRES: Either Finish() or Abandon() has been called.
  ~TableEditor();

  void AddPair(const Slice &key, const Slice &value);
 ...
 private:
  ...
  const uint64_t level_;
  std::string *old_filter_;

  uint64_t kBufferSize = 1ul << 20;  // 1 MB
  ...
  FilterTableBuilder *filter_table_;
